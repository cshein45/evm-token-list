...
6. **Submit PR** (if tests pass):
   - Create branch: `add-token-{SYMBOL}`
...
### Decimal Edge Cases
- **Range Validation**: Ensure decimals between 0-36 (practical ERC-20 range)
- **Type Validation**: Confirm decimals is integer, not string
- **Common Values**: Flag unusual decimal values (not 18, 6, 8) for review

### Multi-Token Issues
- **Batch Requests**: Handle issues requesting multiple tokens
- **Split Processing**: Create separate branches/PRs for each token
- **Dependencies**: Handle tokens that depend on each other
